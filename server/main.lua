...
-- I redid the way to save the player in the database from line 9 to 20 only
...
AddEventHandler('qb-clothing:saveSkin', function(model, skin)
    ...
    local result = MySQL.Sync.fetchAll('SELECT * FROM playerskins WHERE citizenid = ?', {Player.PlayerData.citizenid})
       if result[1] ~= nil then 
            MySQL.Async.execute('UPDATE playerskins SET model = ?, skin = ?, active = ? WHERE citizenid = ?',{model, skin, 1, Player.PlayerData.citizenid})
       else 
