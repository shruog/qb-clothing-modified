		...
		local dist = #(pos - Config.Stores[k].coords)
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - para trocar suas roupas')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - para cortar o cabelo')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - para fazer cirurgia plastica')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                ...
                                elseif Config.Stores[k].shopType == "barber" then
                                    ...
                                    -- })
                                    TriggerEvent("qb-charcreator:openCharCreatorMenu", "editing")
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Meus trajes", selected = true, outfits = result},
        })
    end)
end)
...
function GetMaxValues()
    maxModelValues = {
        ...
        ["hair"]        = {type = "hair", item = 0, texture = 0, texture2 = 0},
        ...
        ["nose_0"]       = {type = "hair", item = -1.0, texture = 0},
        ["nose_1"]       = {type = "hair", item = -1.0, texture = 0},
        ["nose_2"]       = {type = "hair", item = -1.0, texture = 0},
        ["nose_3"]       = {type = "hair", item = -1.0, texture = 0},
        ["nose_4"]       = {type = "hair", item = -1.0, texture = 0},
        ["nose_5"]       = {type = "hair", item = -1.0, texture = 0},
        ...
        ["makeup"]      = {type = "hair", item = 0, texture = 0, texture2 = 0},
...
function resetClothing(data)
    ...
    SetPedHairColor(ped, data["hair"].texture, data["hair"].texture2)

    -- Eyebrows
    SetPedHeadOverlay(ped, 2, data["eyebrows"].item, data["eyebrows"].opacity)
    ...
    -- Blush
    SetPedHeadOverlay(ped, 5, data["blush"].item,  data["blush"].opacity)
    SetPedHeadOverlayColor(ped, 5, 1, data["blush"].texture, 0)

    -- Lipstick
    SetPedHeadOverlay(ped, 8, data["lipstick"].item, data["lipstick"].opacity)
    SetPedHeadOverlayColor(ped, 8, 1, data["lipstick"].item, 0)

    -- Makeup
    SetPedHeadOverlay(ped, 4, data["makeup"].item, data["makeup"].opacity)
    SetPedHeadOverlayColor(ped, 4, 1, data["makeup"].texture, data["makeup"].texture2)

    -- Ageing
    SetPedHeadOverlay(ped, 3, data["ageing"].item, data["ageing"].opacity)
...
AddEventHandler('qb-clothing:client:loadPlayerClothing', function(data, ped)
    ...
    -- Makeup
    SetPedHeadOverlay(ped, 4, data["makeup"].item,  data["makeup"].opacity)
    SetPedHeadOverlayColor(ped, 4, 2, data["makeup"].texture, data["makeup"].texture2)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        QBCore.Functions.Notify("Você escolheu "..oData.outfitName.."! Pressione em confirmar para escolher esta roupa.")
    end
end)
